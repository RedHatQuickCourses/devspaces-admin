...
. You need a list of container images that you need the image puller to pre-pull before workspace start. 

. In OpenShift Dev Spaces 3.27, the legacy embedded plugin registry is deprecated in favor of the embedded Open VSX registry. The older `external_images.txt` endpoint might not be available or maintained. Instead, manually specify the workspace images that should be pre-pulled by the Kubernetes Image Puller.
+
```bash
registry.redhat.io/devspaces/udi-rhel9
registry.redhat.io/devspaces/traefik-rhel9
```
...
...
    ...
    imagePuller:
      ...
      spec: 
        images: >- <2>
          registry.redhat.io/devspaces/udi-rhel9;registry.redhat.io/devspaces/traefik-rhel9
        ... 
----
+
